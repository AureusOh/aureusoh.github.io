...
`matplotlib.pyplot`과 `numpy`를 임포트해줍니다. 일반적으로 `plt`과 `np`을 이름으로 사용합니다.
...
sine 함수를 그려보도록 하겠습니다. x값과 y값을 만들어줍니다. `numpy`의 `linspace`를 이용하면 등간격의 배열을 쉽게 만들어 줄 수 있습니다. `np.sin()` 함수를 이용하여 y값을 계산해줍니다.
...
<pre>[&lt;matplotlib.lines.Line2D at 0x7ff62942e400&gt;]</pre>
</div>
</div>
<div class="jp-OutputArea-child">
...
`plot` 함수를 이용하여 간단하게 sine 그래프를 그릴 수 있습니다.
